...
def plot_rational(Num, Den, x_min=-1, x_max=1, x_num=100):
    ...
    y_Num = np.polyval(Num, x);
    y_Den = np.polyval(Den, x);
    y = y_Num / y_Den
    plt.plot(x, y);
    plt.axis('tight');
    plt.grid(True)
...
def Chebyshev_gen(N, poles):
    ...
    if (len(poles) == 0):
        ...
        P = np.poly(poles)
        Den = np.reshape(P, (len(P), -1))  # denominator
        ...
        for j in range(1, len(poles)):
            if (np.isinf(poles[j])):
                ...
                tmp2 = sp.simplify(V)
        U = tmp1;
        Num = sp.Poly(U, x).all_coeffs()  # numerator
        Num = I_to_i(Num)
...
def plot_mag(eps, eps_R, F, P, E, w_min=-2, w_max=2, w_num=500, dB=True,
    ...
    P = s_to_w(P);
    E = s_to_w(E)
    F_val = np.polyval(F, w);
    P_val = np.polyval(P, w);
    E_val = np.polyval(E, w)
    S11 = (1 / eps_R) * (F_val / E_val);
    S21 = (1 / eps) * (P_val / E_val)
    if (plot):
        if (dB):
...
def rotation_R(N, i, j, tht):
    ...
    """
    R = np.eye(N)
    R[i - 1, i - 1] = np.cos(tht);
    R[j - 1, j - 1] = np.cos(tht)
    R[i - 1, j - 1] = -np.sin(tht);
    R[j - 1, i - 1] = np.sin(tht)
    return R
...
def annihilation(p, q, i, j, N, M):
    ...
    if (p == q):
        if (p == i):
            print(p == i)
            tht = np.arctan((-M[i - 1, j - 1] + np.sqrt(M[i - 1, j - 1] ** 2 - M[i - 1, i - 1] * M[j - 1, j - 1]) / M[
                j - 1, j - 1]))
        else:
            print('p!=i')
            tht = np.arctan((M[i - 1, j - 1] + np.sqrt((M[i - 1, j - 1] ** 2 - M[i - 1, i - 1] * M[j - 1, j - 1]))) / M[
